...
newFile  = append(regexprep(file, '\.asc$', ''), 'Deakt'); %Dateiname Bild.
text(0.2, 0.9*(max(value)), [S1 S2 S3 S4 S5 S6 S7],'Interpreter','latex', 'FontSize',9);
%text(0.2, 0.6*(max(value)), S, 'Interpreter','latex' );
text(0.2, 0.8*(max(value)),sprintf('\\tau_{H} = %0.1f ms',ftau(B(2))*1000), 'FontSize',9)
text(0.2, 0.6*(max(value)),sprintf('r^2: %0.2f',rsquared), 'FontSize',9)
xlabel('t (s)') 
ylabel('f(t)')
